...
class AssignProjectController extends Controller
{
    ...
    {
        $assign = AssignProject::select('assign_projects.*','assign_projects.user_id as assign_projects_user_id','users.*','users.id as user_id','projects.*','projects.id as project_id', 'streams.*','streams.id as stream_id','streams.user_id as streams_user_id')
