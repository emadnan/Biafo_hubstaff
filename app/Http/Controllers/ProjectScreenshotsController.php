...
class ProjectScreenshotsController extends Controller
{
    ...
    public function getReportsWithDateRange($user_id, $fromdate, $todate)
    {
        ...
        $hours = ProjectScreenshots::where('user_id', $user_id)->whereBetween('date', [$fromdate, $todate])->sum('hours');
        $minutes = ProjectScreenshots::where('user_id', $user_id)->whereBetween('date', [$fromdate, $todate])->sum('minutes');
        $seconds = ProjectScreenshots::where('user_id', $user_id)->whereBetween('date', [$fromdate, $todate])->sum('seconds');
    ...
    public function getAllUsersByCompanyIdAndDates($companyID, $date1, $date2)
    {
        if (is_null($companyID) || $companyID === 0 || is_null($date1) || is_null($date2)) {
            return response()->json(['error' => 'companyID and dates are required and must be valid'], 400);
        }
        ...
        foreach ($projectScreenshots as $screenshot) {
            ...
            $data['id'] = $screenshot->user_id;
            $data['name'] = $screenshot->name;
            $data['company_id'] = $screenshot->company_id;
